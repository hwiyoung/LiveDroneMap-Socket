...
class BaseGeoreferencer(ABC):
    @abstractmethod
    def georeference(self, my_drone):
        """
...
class DirectGeoreferencer(BaseGeoreferencer):
    def georeference(self, my_drone):
        ...
        res = direct_georeferencer.georeference(my_drone)
        return res


class DirectGeoreferencerFlightRPY(BaseGeoreferencer):
    def georeference(self, my_drone):
        R_ypr = self.__A2R_Multi(my_drone.init_eo[5] * math.pi / 180, my_drone.init_eo[4] * math.pi / 180, my_drone.init_eo[3] * math.pi / 180, my_drone.io['comb'])
        R_opk = R_ypr.dot(my_drone.io['R_CB'])
        adjusted_opk = self.__R2A_OPK(R_opk)
        adjusted_eo = np.array([my_drone.init_eo[:3], adjusted_opk]).ravel()
        return adjusted_eo
...
class DirectGeoreferencerGimbalRPY(BaseGeoreferencer):
    def georeference(self, my_drone):
        # Gimbal
        adjusted_opk = self.__rpy_to_opk(my_drone.init_eo[3:])
        adjusted_eo = np.array([my_drone.init_eo[:3], adjusted_opk]).ravel()
        return adjusted_eo
